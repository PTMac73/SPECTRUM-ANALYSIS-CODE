...
from functionWriteInputFile import *
...
for i in range(0,len(energy)):
	...
	for j in range(0,len(J)):
		if j != len(J) - 1:
			...
			writePtolemyBlock(inFile,reactionFullName,JP[j],L[j],energy[i],ELAB,node[j],s,Asymptopia,0)
		else:
			# Last block, so need to write end of the file as well
			writePtolemyBlock(inFile,reactionFullName,JP[j],L[j],energy[i],ELAB,node[j],s,Asymptopia,1)
	# Close the file
	inFile.close()
